...
public interface EnhancedExecutor extends Executor {
	 ...
	 * <br>
	 * The command may execute in the calling thread, but not synchronously. For example, actions submitted from
	 * within one of the threads in <code>ForkJoinPool.commonPool</code> may run later on the same pooled thread.
	 *
